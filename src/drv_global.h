...
#define RS422_RX_MAX (0x0240)

#define RS422_TX_01 (0x0250)
#define RS422_TX_02 (0x0260)
#define RS422_TX_03 (0x0270)
#define RS422_TX_04 (0x0280)
#define RS422_TX_05 (0x0290)
#define RS422_TX_06 (0x02A0)
#define RS422_TX_07 (0x02B0)
#define RS422_TX_08 (0x02C0)
#define RS422_TX_09 (0x02D0)
#define RS422_TX_MAX (0x02D0)

#define ARINC429_RX_01 (0x02E0)
#define ARINC429_RX_02 (0x02F0)
#define ARINC429_RX_03 (0x0300)
#define ARINC429_RX_04 (0x0310)
#define ARINC429_RX_05 (0x0320)
#define ARINC429_RX_06 (0x0330)
#define ARINC429_RX_07 (0x0340)
#define ARINC429_RX_08 (0x0350)
#define ARINC429_RX_MAX (0x0350)

#define ARINC429_TX_01 (0x0360)
#define ARINC429_TX_02 (0x0370)
#define ARINC429_TX_03 (0x0380)
#define ARINC429_TX_MAX (0x0380)

#define ARINC429_100K 100000
...
#define IO_26 (0X0001) << 25
#define IO_27 (0X0001) << 26
#define IO_28 (0X0001) << 27
#define IO_29 (0X0001) << 28
#define IO_30 (0X0001) << 29
#define IO_31 (0X0001) << 30
#define IO_32 (0X0001) << 31
 ...
 * @parity: 校验位
        ...
        RS232_PARITY_EVEN  偶校验
        RS232_PARITY_ODD   奇校验
 ...
 * @parity: 校验位
        ...
        RS422_PARITY_EVEN  偶校验
        RS422_PARITY_ODD   奇校验
...
/**
 *io_read_output_status 读取端口设置的输出值
 *返回值：端口设置的输出值
 */
extern uint32_t io_read_output_status();

/**
 * digital_io_open - 打开离散量端口
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t digital_io_open();

/**
 * digital_io_close - 关闭离散量端口
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t digital_io_close();

/**
 * io_out_high - 设置端口的输出值为高
 * @pin: 需要设置为高电平的端口
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t io_out_high(uint32_t pin);

/**
 * io_out_low - 设置端口的输出值为低
 * @pin: 需要设置为低电平的端口
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t io_out_low(uint32_t pin);
 ...
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t io_out(uint32_t pin);
/**
 * io_set_outputmode -设置端口方向为输出(需要取反即输出0X00001
 * 对应函数为io_out(~0x0001))
 * @pin: 离散量端口编号 取值范围为 IO_01~IO_16
 *
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t io_set_outputmode(uint32_t pin);
/**
 ...
 * 执行成功返回0，其它值则执行错误。
 */
extern int32_t io_set_inputmode(uint32_t pin);
